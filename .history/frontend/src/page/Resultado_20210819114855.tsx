...
const nota = ( (resultadoGeografia.length + resultadoHistoria.length + resultadoMatematica.length ) * 8.34 )
      console.log( resultadoGeografia.length + resultadoHistoria.length + resultadoMatematica.length  )
 return(
  <div className = 'divResultado'>
     <Header titulo = 'Teste de Conhecimentos' />
     <h2 className = 'h2Resultado'> {nota > 70 ?   `Parabens! Você tirou ${ Math.round(nota)} ` : `Infelizmente Vocẽ tirou ${Math.round(nota)}`} </h2>
     <ContainerMain titulo = 'O teste vale 100 pontos, e a nota necessária é 70'>
          <div className= 'divResultadoGrafico'>
              ...
              <div>
                    ...
                    <div className = 'divCont'>  <div style = {respGraficoMat}  ></div>  {resultadoMatematica.length} </div>
              </div>
             <div>
