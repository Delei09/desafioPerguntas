         ...
         {questao.options.map( (option, i) => {
            return(
               <div className = 'divInputLabel'>
                    <input  type="radio" id = {`mat${option}`}onChange = {(e: React.ChangeEvent<HTMLInputElement>)=> {
                     setRespostaMatematica({...respostaMatematica , [questao.id] : option  } )               } }
                  name={`mat${questao.id}`} value= {option} />   
                    <label key = {i}  htmlFor= {`mat${option}`} >  {option}  </label>
...
{perguntasHistoria.map((questao, i) => {
      return(
         ...
         {questao.options.map( (option, i) => {
            return(
               <div className = 'divInputLabel'>
                    <input  type="radio" id = {questao.pergunta} onChange = {(e: React.ChangeEvent<HTMLInputElement>)=> {
                     setRespostaMatematica({...respostaMatematica , [questao.id] : option  } )               } }
                  name={`mat${questao.id}`} value= {option} />   
                    <label  htmlFor= {questao.pergunta}>  {option}  </label>
         ...
         {questao.options.map( (option, i) => {
            return(
               ...
               <input  type="radio" id = {questao.pergunta} onChange = {(e: React.ChangeEvent<HTMLInputElement>)=> {
                setRespostaMatematica({...respostaMatematica , [questao.id] : option  } )               } }
             name={`mat${questao.id}`} value= {option} />   
               <label key = {i}  htmlFor= {questao.pergunta}>  {option}  </label>
