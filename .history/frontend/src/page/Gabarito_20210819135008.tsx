...
{perguntasHistoria.map((questao, i) => {
      return(
         ...
         {questao.options.map( (option, i) => {
            return(
               <label key = {i}>  <input name={`his${questao.id}`} type="radio" defaultChecked = {option === questao.resposta}  id = {questao.id} /> {option}  </label>
            )
         })}
         </div>
         {i < 3 ? <hr/> : ''} 
...
{perguntasGeografia.map((questao, i) => {
      return(
         ...
         {questao.options.map( (option, i) => {
            return(
               <label key = {i}>  <input name={`geo${questao.id}`}type="radio" id = {questao.id}  defaultChecked = {option === questao.resposta}   /> {option}  </label>
            )
         })}
         </div>
         {i < 3 ? <hr/> : ''} 
