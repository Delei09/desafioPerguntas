...
const Home : React.FC= () => {
   ...
   const [respostaMatematicaContext , setRespostaMatematicaContext] = useState<respostaType>({} as respostaType)
   const [respostaHistoriaContext , setRespostaHistoriaContext] = useState<respostaType>({} as respostaType)
   const [respostaGeografiaContext , setRespostaGeografiaContext] = useState<respostaType>({} as respostaType)
type respostaType = {
   p1 : string ,
   p2 : string ,
   p3 : string ,
...
{perguntasHistoria.map((questao, i) => {
      return(
         ...
         {questao.options.map( (option, i) => {
            return(
               <label key = {i}>  <input name = {questao.id}  type="radio" id = {questao.id} onChange = {(e: React.ChangeEvent<HTMLInputElement>)=> {
