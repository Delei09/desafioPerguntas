         ...
         {questao.options.map( (option, i) => {
            return(
             ...
             name={`mat${questao.id}`} value= {option} />   
               <label className = 'label'  key = {i}  htmlFor= {questao.id+option}>  {option}  </label>
...
{perguntasGeografia.map((questao, i) => {
      return(
         ...
         {questao.options.map( (option, i) => {
            return(
               <div className = 'divInputLabel' key = {option }  >
               <input  type="radio" id = {questao.id+option} defaultChecked = {    (option === questao.resposta) || (option === respGeo.p1)    } 
             name={``} value= {option} />   
               <label key = {i}  htmlFor= {questao.id+option}>  {option}  </label>
